...
class HelperAPI(grok.XMLRPC):
    ...
    def getFlgIds(self, teilnehmer_id):
        ...
        ret = session.query(Kursteilnehmer.fernlehrgang_id).filter(
            Kursteilnehmer.teilnehmer_id == teilnehmer_id, Kursteilnehmer.status != "Z1"
        )
        return ret.all()
    ...
    def getMNRFromTeilnehmerID(self, teilnehmer_id):
        ...
        session = Session()
        ret = session.query(Teilnehmer.unternehmen_mnr).filter(
...
class TeilnehmerAPI(grok.REST):
    ...
    def PUT(self):
        ...
        data["geburtsdatum"] = datetime.datetime.strptime(
            data["geburtsdatum"], "%d.%m.%Y"
        )
