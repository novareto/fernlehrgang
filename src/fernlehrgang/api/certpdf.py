...
def createpdf(filehandle, data):
    ...
    image = (
        pathlib.Path(fernlehrgang.api.__file__).resolve().parent
        / "FLG_Bescheinigung_Ansicht_Image.jpg"
    )
    c = canvas.Canvas(filehandle, pagesize=A4)

    # Metainformationen fuer das PDF-Dokument
    c.setAuthor("Berufsgenossenschaft Handel und Warenlogistik")
    c.setTitle("Teilnahmezertifikat Fernlehrgang")
    ...
    c.drawString(13.8 * cm, 23.52 * cm, data.get("flg_titel", " "))
    c.drawString(
        13.8 * cm, 22.92 * cm, "Benutzer-Nr.: %s " % data.get("teilnehmer_id", " ")
    )

    fullname = "%s %s %s %s" % (
        anrede[data.get("anrede", "1")],
        titel[data.get("titel", "0")],
        data.get("vorname"),
        data.get("name"),
    )
    ...
    try:
        dd = datetime.strptime(data.get("druckdatum"), "%d.%m.%Y").strftime(
            "Bonn, %d. %B %Y"
        )
    except:
...
def createfortpdf(filehandle, data):
    ...
    image = (
        pathlib.Path(fernlehrgang.api.__file__).resolve().parent
        / "FLG_Fortbildung_Ansicht_Image.jpg"
    )
    timestamp = strftime(
        "%d%m%Y%H%M%S", gmtime()
    )  # Ermitteln der aktuellen Uhrzeit und Formatierung eines Strings mit Zeitstempel
    ...
    c.drawString(14.6 * cm, 23.6 * cm, data.get("flg_titel", " "))
    c.drawString(
        14.6 * cm, 23.0 * cm, "Benutzer-Nr.: %s" % data.get("teilnehmer_id", " ")
    )
    ...
    c.setFont(schriftart, 12)
    c.drawString(2.2 * cm, 5.5 * cm, "Bonn, %s" % data.get("certdatum"))
