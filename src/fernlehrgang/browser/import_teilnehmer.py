...
from fernlehrgang.exports.utils import page_query, getUserEmail
...
def createXLS(mail, flg_id, tids, flgids):
    ...
    i = 0
    result = (
        session.query(models.Teilnehmer, models.Unternehmen, models.Kursteilnehmer)
...
def importUsers(mail, flg_id, tids, flgids, context_id):
    ...
    with transaction.manager:
        ...
        send_mail(
            ...
            (mail,),
            "Import Erfolgreich",
            "Import Erfolgreich %s Teilnehmer importiert, Von %s  --> Nach %s"
            % (i, flg_id, context_id),
        )
...
class ImportTeilnehmer(Page):
    ...
    def render(self):
        filename = os.path.join(
            os.path.dirname(__file__), "templates", "import_teilnehmer.cpt"
        )
    ...
    def __call__(self):
        ...
        flgids = [
            x[0]
            for x in session.query(models.Fernlehrgang.id)
            .filter(models.Fernlehrgang.typ == self.context.typ)
            .all()
            if x[0] != int(key) and x[0] != self.context.id
        ]
        ...
        if action == "import":
            ...
            print("ADD STUFF TO THE QUEUE")
            q.enqueue_call(
                func=createXLS, args=(mail, flg_id, tids, flgids), timeout=19800
            )
