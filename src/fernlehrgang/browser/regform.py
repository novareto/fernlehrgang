...
class AutoRegForm(Form):
    ...
    def getContentData(self):
        ...
        if len(mnr_or_unrs) == 15:
            session = Session()
            unternehmen = (
                session.query(Unternehmen)
                .filter(Unternehmen.unternehmensnummer == mnr_or_unrs)
                .one()
            )
            mnr = unternehmen.mnr
        else:
    ...
    def handle_save(self):
        ...
        if not unternehmen:
            self.flash(
                "Das Unternehmen mit der Mitgliedsnummer %s existiert nicht"
                % data.get("mnr")
            )
            return
        tn = Teilnehmer(
        ...
        self.flash(
            "Der Teilnehmer wurde als Kursteilnehmer mit der ID %s angelegt." % tn.id
        )
