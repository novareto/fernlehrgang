...
def createRows(session, rc, flg_ids, stichtag):
    ...
    for x in ids:
        lehrhefte_sql = session.query(models.Lehrheft).options(
            joinedload(models.Lehrheft.fragen)
        )
    ...
    for teilnehmer, unternehmen, ktn in result.all():
        if ktn.status in ("A1", "A2"):
            ...
            if teilnehmer and summary.get("resultpoints") >= 1:
                ...
                liste.append(
                    nN(ktn.fernlehrgang.lehrhefte[0].id)
                )  # IST IMMER NUR EINS DA--Fortbilding
                ...
                for lehrheft in ktn.fernlehrgang.lehrhefte:
                    for frage in sorted(
                        lehrheft.fragen, key=lambda frage: int(frage.frage)
                    ):
                        r = ""
...
class XLSFortbildung(Form):
    ...
    grok.title("Versandliste Fortbildung")
    # grok.baseclass()
